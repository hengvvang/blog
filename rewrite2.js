...
// 1. Rename renderToolchainPartition to renderPartition
text = text.replace('function renderPartition() {', 'function renderPartition_OLD() {'); // Rename the old one if it exists
text = text.replace('function renderToolchainPartition() {', 'function renderPartition() {');

// 2. We need to replace the entire renderPartition function body.

const newFunc = `function renderPartition() {
...
// Use AST or simple index replacements if possible
const startStr = "function renderPartition() {";
const endStr = "(window as any).selectToolchainSubcat = (sub: string) => {";
const startIdx = text.indexOf(startStr);
const endIdx = text.indexOf(endStr);
if(startIdx !== -1 && endIdx !== -1) {
    text = text.substring(0, startIdx) + newFunc + "\n\n" + text.substring(endIdx);
}

// 3. Fix loadMore bindings
text = text.replace(/selectToolchainSubcat/g, "selectSubcat");
text = text.replace(/loadMoreToolchain/g, "loadMore");

// 4. Update the renderArticles function
const renderArticlesStart = text.indexOf("} else if (currentCategory === 'toolchain') {");
const renderArticlesEnd = text.indexOf("}\n  }\n\n  // 首次渲染");
if(renderArticlesStart !== -1 && renderArticlesEnd !== -1) {
    const replacement = `} else {
      renderPartition();
    }`;
    text = text.substring(0, renderArticlesStart) + replacement + text.substring(renderArticlesEnd);
}
