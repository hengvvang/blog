...
text = text.replace(/let currentToolchainSubcat = 'all';/g, `let currentSubcat = 'all';`);
text = text.replace(/let toolchainPageSize = 5;/g, `let pageSize = 5;`);
text = text.replace(/currentToolchainSubcat/g, 'currentSubcat');
text = text.replace(/toolchainPageSize/g, 'pageSize');

const defBodyMatch = text.match(/function renderToolchainPartition\(\) \{[\s\S]*?\n\}\n/);
if (defBodyMatch) {
...
`;
  text = text.replace(defBodyMatch[0], newFunc);
}

text = text.replace(/window\.selectToolchainSubcat/g, "window.selectSubcat");
text = text.replace(/window\.loadMoreToolchain/g, "window.loadMore");
text = text.replace(/renderToolchainPartition\(\)/g, "renderPartition()");

const renderArticlesRegex = /\} else if \(currentCategory === 'toolchain'\) \{[\s\S]*?\}\n  \}\n/;
const renderArticlesMatch = text.match(renderArticlesRegex);
if (renderArticlesMatch) {
  const replacement = `} else {
    renderPartition();
  }
}
`;
  text = text.replace(renderArticlesMatch[0], replacement);
}
