...
随着响应式设计和移动端的兴起，布局系统的灵活性（Flexibility）与控制力（Control）成为了现代 Web 开发的刚需。一维弹性盒（Flexbox）与二维网格（Grid）布局规范随之应运而生。
...
| **重叠需求** | 难以优雅处理元素层叠，需借助相对/绝对定位 | 原生支持不同子项分配到相同/交叉的网格线，实现完美重叠 |
...
本指南致力于提供生产级的技术深度，避免流于属性的简单罗列。在接下来的两大部分中，我们将由浅入深攻克以下核心知识体系：

```
[第一部分：Flexbox 弹性布局] ─────────> [第二部分：Grid 网格与响应式]
    ├── 主轴与交叉轴的动态旋转              ├── 显式网格与隐式网格定义
    ├── 伸缩因子正负向计算数学公式           ├── fr 轨道单位底层解析与 gap 解析
    ├── 双向对齐与 margin-auto 妙用          ├── 密集排列算法与 Template Areas 坐标
    └── 生产级 Flexbox 组件模板             └── auto-fit/minmax() 响应式与容器查询
```

### 第一部分：Flexbox 布局弹性机制
我们将剖析 Flex 容器的坐标变换、主轴与交叉轴的旋转及书写模式控制，深入推导浏览器在处理剩余空间分配（`flex-grow`）以及空间不足压缩（`flex-shrink`）时的精确数学公式与权重加权算法，并提供多个组件级的 CSS 布局模版。

### 第二部分：Grid 布局架构与响应式设计
我们将深入研究 CSS Grid 的多轴轨道划分，对比 `fr` 弹性轨道单位与传统百分比单位在解析上的本质差异。学习显式网格（Explicit Grid）与隐式网格（Implicit Grid）的转换，掌握网格区域命名、重叠定位以及自动流（`grid-auto-flow: dense`）的密集填充算法。同时，我们将探讨如何利用 Grid 的 `auto-fit`/`auto-fill` 配合 `minmax()` 函数构建高流动性的自适应网格，并对 CSS Subgrid（子网格）对齐技术及 CSS 容器查询（Container Queries）展开实战说明。
