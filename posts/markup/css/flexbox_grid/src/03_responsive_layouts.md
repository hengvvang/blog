# 第三章：响应式布局：无媒体查询的现代响应式实践

在响应式设计的早期，开发者严重依赖于媒体查询（Media Queries, `@media`）来硬编码设备宽度断点（如 `320px`, `768px`, `1024px`）。然而，在现代高度组件化的前端工程中，这种“视口依赖型”设计逐渐暴露出诸多缺陷。

## 1. 传统媒体查询的痛点与局限

* **高昂的维护成本**：当页面断点增加时，CSS 文件会充斥大量的媒体查询块，代码结构极易破碎。
* **组件重用性差**：一个原本在侧边栏（宽度小）和主内容区（宽度大）都能渲染的卡片组件，若使用 `@media` 查询，它只能根据屏幕宽度（视口）进行缩放，而无法感知其父容器容器物理尺寸的变化。
...
### 深度辨析：`auto-fill` vs `auto-fit`

当网格容器的宽度非常宽，以至于容纳完所有网格项后，**仍有大量多余空间**时，这两个关键字的行为走向了完全不同的分叉路口。

假设容器宽度为 **1200px**，间距为 **0**，我们有 3 个网格项，每个子项的最小尺寸（`minmax` 中的下限）定为 **200px**。此时，容器理论上最多可以容纳 $1200 / 200 = 6$ 列。

#### 1. `auto-fill`（自动填充空轨道）
* **逻辑**：浏览器会坚持在 1200px 宽度内划分出 6 个物理轨道（即使只有 3 个元素）。
* **结果**：前 3 列摆放子项，每个子项的实际宽度刚好是其设定的拉伸值。后 3 列则以“空轨道”的形式保留在右侧，元素不会强行横向铺满整个 1200px。
* **结构呈现**：
```
[ Item 1 ] [ Item 2 ] [ Item 3 ] [ 空轨道 ] [ 空轨道 ] [ 空轨道 ]
└─────────── 占用 200px ───────────┘└───────── 保留空余轨 ─────────┘
```

#### 2. `auto-fit`（自动合并空轨道）
* **逻辑**：浏览器同样会计算出 6 个轨道，但一旦发现后 3 个轨道没有任何网格项，它会**强行将这些空轨道的宽度折叠为 0px**（相当于它们不存在）。
* **结果**：原本由 6 个轨道瓜分的 1200px 空间，现在仅由 3 个非空轨道均分。因此，每个子项的宽度会被拉伸至 $1200 / 3 = 400px$。
* **结构呈现**：
```
...
### `minmax(300px, 1fr)` 动态解析算法

当浏览器渲染 `.fluid-grid` 容器时，它执行以下算法流程：

1. **读取容器可用主轴宽度**：$W_{avail}$。
2. **计算最大列数** $N$：
   考虑 gap 的物理占用，计算最多能排下多少个 `300px` 列。
   $$N = \lfloor \frac{W_{avail} + Gap}{300px + Gap} \rfloor$$
3. **分配列轨**：
   * 若使用 `auto-fit`：将多余空轨宽度归零，由实际网格项数量 $M$（$M \le N$）均分主轴宽度。每列最终宽度 $W_{col} = \frac{W_{avail} - (M-1) \times Gap}{M}$。
   * 若使用 `auto-fill`：保持 $N$ 列轨道。每列最终宽度 $W_{col} = \frac{W_{avail} - (N-1) \times Gap}{N}$。
4. **流动换行**：随着视口缩小，当公式算出的 $N$ 变小时，浏览器自动将多出的卡片折行排列。
...
.parent-grid {
  ...
  grid-template-rows: auto auto auto; /* 定义三行轨道 */
}

.card-item {
  grid-row: span 3; /* 每张卡片跨越 3 行轨道 */
  display: grid;
  grid-template-rows: subgrid; /* 行轨直接对齐父级网格线 */
}
...
容器查询是响应式布局的革命性突破。它允许组件**基于其父容器的宽度**（而不是整个浏览器视口的宽度）来改变自身的样式。
...
.card-parent-container {
  ...
  container-name: sidebar-or-main; /* 可选：命名容器 */
}
```
...
.dashboard-body {
  display: flex;
  flex-wrap: wrap; /* 允许侧边栏和主内容在小屏下折行 */
  flex-grow: 1;
}
...
.gallery-wrapper {
  display: grid;
  /* 保证单个商品项宽度至少为 240px，并在多余空间内拉伸填满 */
