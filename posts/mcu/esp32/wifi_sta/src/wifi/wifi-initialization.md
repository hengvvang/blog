# 第二章：Wi-Fi 站模式（STA）初始化与配置流程

将 ESP32 配置并启动为客户端工作站模式（STA 模式）并非只是简单地调用一个 `connect` 函数。从系统级角度来看，这是一个包含非易失性存储加载、网络协议适配器绑定、底层射频（RF）校准和安全机制协商等多步骤、紧密耦合的初始化链路。

本章将详细剖析 Wi-Fi 初始化生命周期中的八个核心步骤、底层的射频（RF）校准机制、低功耗省电机制，以及生产级初始化代码的实现。

---

## 2.1 逐步剖析 Wi-Fi 初始化全生命周期

在 ESP-IDF v5.x 中，Wi-Fi STA 模式的标准初始化流程如下图所示：

```
+---------------------------------------------------------------------------------------------------+
|                                  Wi-Fi STA 初始化生命周期全流程                                     |
+---------------------------------------------------------------------------------------------------+
|                                                                                                   |
|  [ 1. nvs_flash_init() ] -----------------> 加载 PHY 校准参数，读取或重建 NVS 闪存分区。            |
|            |                                                                                      |
|            v                                                                                      |
|  [ 2. esp_netif_init() ] -----------------> 初始化 esp_netif 适配框架，为协议栈分配资源。           |
|            |                                                                                      |
|            v                                                                                      |
|  [ 3. esp_event_loop_create_default() ] --> 启动 sys_evt 任务，创建核心事件分发队列。               |
|            |                                                                                      |
|            v                                                                                      |
|  [ 4. esp_netif_create_default_wifi_sta() ]  创建 STA 适配层句柄，绑定 DHCPv4 客户端。            |
|            |                                                                                      |
|            v                                                                                      |
|  [ 5. esp_wifi_init(&init_cfg) ] ---------> 启动 libwifi.a 驱动，分配 DMA 描述符并挂载 PHY。        |
|            |                                                                                      |
|            v                                                                                      |
|  [ 6. esp_wifi_set_mode(WIFI_MODE_STA) ] -> 将网卡模式锁定为 Station (STA)。                       |
|            |                                                                                      |
|            v                                                                                      |
|  [ 7. esp_wifi_set_config(WIFI_IF_STA) ] -> 配置 SSID、密码、监听周期 (DTIM) 以及 PMF/WPA3 安全选项。|
|            |                                                                                      |
|            v                                                                                      |
|  [ 8. esp_wifi_start() ] -----------------> 激活 RF 锁相环，硬件开始扫描信道并投递 START 事件。    |
|                                                                                                   |
+---------------------------------------------------------------------------------------------------+
```

### 步骤 1：NVS 闪存初始化 (`nvs_flash_init`)
**为什么 Wi-Fi 深度依赖 NVS（Non-Volatile Storage）？**
1.  **PHY 校准参数的持久化**：ESP32 的射频物理层在出厂或首次启动时，需要对天线阻抗、中心频率偏置、发射功率以及接收增益进行精确校准。这些数据被称为 PHY 校准数据（存储在 `phy` 分区中，并通过 NVS 进行校验管理）。如果 NVS 初始化失败，底层 PHY 驱动将无法加载，芯片通常会触发 Panic 重启。
2.  **配网数据与状态的持久化**：Wi-Fi 驱动在运行过程中会产生信道历史记录（Channel History）、AP 的 BSSID 缓存以及射频温漂补偿值。将这些数据存入 NVS 可以使设备在断电重启后，跳过全信道扫描，直接在上次连接的信道上建立关联，从而将建连时间从 3 ~ 5 秒缩短至 1 秒以内。
